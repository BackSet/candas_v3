...
| Ensacado | `/ensacado`; `hooks/useEnsacado.ts`, `hooks/useBarcodeScanner.ts`; `components/scanner/CaptureModeToggle.tsx`; `ensacado.service.ts` | `EnsacadoController`, `EnsacadoService` | `ensacado_sesion`, `saca`, `paquete` | `ensacado:operar` |
...
| Listas etiquetadas | Rutas legacy `/listas-etiquetadas` y `/operario-etiquetas` redirigen; componentes en `components/listas-etiquetadas`; modo operario con captura dual lector/camara; service dedicado | `ListasEtiquetadasController`, `ListasEtiquetadasService` | Flujo basado en `paquete`; tablas historicas de listas fueron eliminadas por migraciones `V113` | Pendiente de confirmar; usa endpoints bajo paquetes |
...
- Lotes Recepcion crea/importa/organiza paquetes; Lotes Especiales reutiliza infraestructura de Lotes Recepcion. Los operadores de lote normal y especial conservan captura, tipeo/clasificacion y guardado en el lote, pero no crean despachos; la creacion de despachos vive en Despachos y Despachos rapidos. [verificado en Git]
...
- Recepcion por lote y tipeo: rutas `/lotes-recepcion*`, endpoints `/api/v1/lotes-recepcion*`, `TipoLote`. Los operadores de lote normal/especial no crean despachos desde esta UI; solo agregan paquetes al lote, clasifican por tipo/destino y permiten revisar/exportar/imprimir segun corresponda. [verificado en Git]
