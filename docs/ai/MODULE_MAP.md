...
- PWA (capacidad transversal): la SPA es instalable como app (Windows/Edge-Chrome, Android, iOS por "Añadir a pantalla de inicio") y abre standalone. Configurada con `vite-plugin-pwa` en `vite.config.ts` (manifest + service worker Workbox, `autoUpdate`). Acción de instalación discreta en `Header.tsx` vía `components/pwa/InstallPrompt.tsx` + `hooks/usePwaInstallPrompt.ts` (capta `beforeinstallprompt`, detecta standalone e iOS). Precache de app shell; sin caché de API ni offline transaccional. Iconos en `public/` (`pwa-*.png`, `apple-touch-icon.png`). [verificado en Git]
